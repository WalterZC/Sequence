...
namespace getAddress.Sequence.Tests
{
    ...
    public class Tests
    {
        ...
        public async Task NextMethodThrowsExceptionIfSequencyCanNotBeFound()
        {
            ...
            SequenceCouldNotBeFoundException scnotfound = await Assert.ThrowsAsync<SequenceCouldNotBeFoundException>( () =>  sequenceGenerator.NextAsync(new SequenceKey { Value = "1"}));
            Assert.NotNull(scnotfound);
        }
        ...
        public async Task NextMethodThrowsExceptionWhenMaximumValueIsReached()
        {
            ...
            MaximumValueReachedException  x = await Assert.ThrowsAsync< MaximumValueReachedException>( () => sequenceGenerator.NextAsync(sequenceKey));
            Assert.NotNull(x);
        ...
        public async Task NextMethodThrowsExceptionWhenMinimumValueIsReached()
        {
            ...
            MinimumValueReachedException miniVal = await Assert.ThrowsAsync<MinimumValueReachedException>(() => sequenceGenerator.NextAsync(sequenceKey));

            Assert.NotNull(miniVal);
        }
        ...
        public async Task NextMethodThrowsExceptionWhenIfMaxRetryAttemptIsReach()
        {
            ...
            MaxRetryAttemptReachedException retryex =  await Assert.ThrowsAsync< MaxRetryAttemptReachedException>( ()=> sequenceGenerator.NextAsync(sequenceKey));
        }
    }
}
